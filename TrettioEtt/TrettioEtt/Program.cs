...
namespace TrettioEtt
{
    ...
    class MyPlayer : Player  //Döp om denna klass till något unikt, max 14 bokstäver, ändra även i main
    {
        // 77% sannolikhet för vinst.
        List<Card> knownCards = new List<Card>();
        List<Card> opponentCards = new List<Card>();
        bool initialize = true;

        public MyPlayer() //Skriv samma namn här
        {
            Name = "MyPlayer"; //Skriv in samma namn här
        }
        ...
        public override bool Knacka(int round) //Returnerar true om spelaren skall knacka, annars false
        {
            ...
            {
                if (OpponentsLatestCard.Value <= 5 && Game.Score(this) > 12)
                {
                    return true;
                }
            }
            if(opponentCards.Count == 2 && GetOpponentMax() < Game.Score(this))
            {
                return true;
            }*/
        ...
        // Last turn om modståndaren har knackat
        // Lägg inte höga kort av en färg om motståndaren har högt av färgen.
        ...
        public override bool TaUppKort(Card card) // Returnerar true om spelaren skall ta upp korten på skräphögen (card), annars false för att dra kort från leken.
        {
            ...
            int highValue = Game.Score(this);

            if (card.Suit == BestSuit)
            {
                if(card.Value > LowestCardOnHand().Value)
                {
                    return true;
                }
                else if(IndexOfSuit(BestSuit) == 2)
                {
                    return true;
                }
                return false;            
            ...
            if (card.Value > LowestCardOnHand().Value)
            {
                ...
                {
                    case Suit.Hjärter:
                        if (card.Value + Game.SuitScore(Hand, Suit.Hjärter) > highValue)
                        {
                            return true;
                        }
                        break;
                    ...
                    case Suit.Spader:
                        if (card.Value + Game.SuitScore(Hand, Suit.Spader) > highValue)
                        {
                            return true;
                        }
                        break;
                    ...
                    case Suit.Klöver:
                        if (card.Value + Game.SuitScore(Hand, Suit.Klöver) > highValue)
                        {
                            return true;
                        }
                        break;
                    ...
                    case Suit.Ruter:
                        if (card.Value + Game.SuitScore(Hand, Suit.Ruter) > highValue)
                        {
                            return true;
                        }
                        break;
        ...
        {
            Game.Score(this);
            if(IndexOfSuit(BestSuit) == 4)
            {
                return (LowestCardOnHand());
            }
            if(IndexOfSuit(BestSuit) == 3)
            {
                return (CardOutsideSuit(BestSuit));
            }
            if(IndexOfSuit(BestSuit) == 2)
            {
                return (LowestCardOutsideSuit(BestSuit));
            }
            if(IndexOfSuit(BestSuit) == 1)
            {
                return LowestCardOnHand();
            }
        ...
        private int GetOpponentMax()
        {
            return (Game.HandScore(opponentCards, null));
        }

        private Card LowestCardOnHand()
        {
        ...
        private Card LowestCardOfSuit(Suit aSuit)
        {
            Card worstCard = null;
            for (int i = 1; i < Hand.Count; i++)
            {
                if(Hand[i].Suit == aSuit)
                {
                    worstCard = Hand[i];
                    break;
                }   
            ...
            for (int i = 1; i < Hand.Count; i++)
            {
                if (Hand[i].Suit == aSuit && Hand[i].Value < worstCard.Value)
                {
                    worstCard = Hand[i];                  
                }
            }
        ...
        private Card CardOutsideSuit(Suit aSuit)
        {
        ...
        private int IndexOfSuit(Suit aSuit)
        {
            int index = 0;
            for (int i = 0; i < Hand.Count; i++)
            {
        ...
        private Card LowestCardOutsideSuit(Suit aSuit)
        {
            Card worstCard = null;
            for (int i = 0; i < Hand.Count; i++)
            {
